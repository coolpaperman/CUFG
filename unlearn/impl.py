...
def save_unlearn_checkpoint(model, evaluation_result, args):
    ...
    utils.save_checkpoint(state, False, args.save_dir, args.unlearn, args.num_indexes_to_replace)
...
def load_unlearn_checkpoint(model, device, args):
    checkpoint = utils.load_checkpoint(device, args.save_dir, args.unlearn, args.num_indexes_to_replace)
...
def _iterative_unlearn_impl(unlearn_iter_func):
    def _wrapped(data_loaders, sub_datasets, model, criterion, args, mask=None, **kwargs):
        ...
        for epoch in range(0, args.unlearn_epochs):
            ...
            train_acc = unlearn_iter_func(
                data_loaders, sub_datasets, model, criterion, optimizer, epoch, args, mask, **kwargs
            )
            scheduler.step()
